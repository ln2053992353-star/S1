...
export class DataProvider {
  ...
  async fetchData() {
    ...
    if (this.config.dataSource === 'embedded') {
      return this._fetchEmbedded();
    }
  ...
  async _fetchEmbedded() {
    var el = document.getElementById('search-data');
    if (!el) {
      throw new Error(
        'Embedded data element #search-data not found in the page. ' +
        'Ensure the Django template includes <script id="search-data" type="application/json">...</script>.'
      );
    }
    var text = (el.textContent || '').trim();
    if (!text) {
      throw new Error('Embedded data is empty.');
    }
    try {
      return JSON.parse(text);
    } catch (e) {
      throw new Error('Embedded data is not valid JSON: ' + e.message);
    }
  }
